...
class Scene {
    ...
    #drawWall(x, color, height, hitValue, slicer, startY, correctedRayDistance) {
        let endY = startY + height + this.observer.player.playerHeight;
        ...
        let wallTexture = this.textureManager.textures.get(hitValue);

        if (Array.isArray(wallTexture)) {
            wallTexture = wallTexture?.[Math.floor(this.animationFrameCounter) % (wallTexture.length)];
        }

        wallTexture = wallTexture.getSliceBuffer(textureX % 32, 0, 1, 32, brightness);
    ...
    #drawFloor(wall, scanRange) {
        for (let y = scanRange.min; y < scanRange.max; y++) {
            ...
            const realDistance = rowDistance / Math.cos((betaAngle - this.observer.player.rotateX) * RADIUS);
            ...
            const bufferPosition = y * this.SCENE_WIDTH + wall.x;
            ...
            const color = floorTexture.getSliceBuffer(floorTextureX, floorTextureY, 1, 1, brightness);
    ...
    #drawCeil(wall, scanRange) {
        for (let y = scanRange.min; y < scanRange.max; y++) {
            const betaAngle = wall.rayAngle;
            const rowDistance = (this.WALL_HEIGHT - this.observer.player.playerHeight) * this.distanceFromPlayerToProjectionPlane / (this.screenMiddleHeight - y);
            const realDistance = rowDistance / Math.cos((betaAngle - this.observer.player.rotateX) * RADIUS);
    ...
    render() {
        ...
        const walls = this.observer.raysHittingPoints.map((ray, index) => {
            const angleDiff = ray.angle - this.observer.player.rotateX;
            const correctedRayDistance = (ray.distance * Math.cos(angleDiff * RADIUS));
            const startY = Math.floor((this.distanceFromPlayerToProjectionPlane / correctedRayDistance) * (this.observer.player.playerHeight - this.WALL_HEIGHT) + this.screenMiddleHeight);
            return {
                x: index,
        ...
        for (const wall of walls) {
            ...
            this.#drawWall(wall.x, 0xFF000000, wall.height, wall.hitValue, wall.slicer, wall.startY, wall.correctedRayDistance);
            this.#drawFloor(wall, { min: Math.floor(wall.startY + wall.height), max: this.SCENE_HEIGHT });
        }
