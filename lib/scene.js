    ...
    render() {
        ...
        for (let i = 0, bufferX = 0; i < this.player.fieldOfViewDeg; i += this.rayStep, bufferX++) {
            ...
            for (let castedBlockIndex = 0; castedBlockIndex < castedBlocksCount; castedBlockIndex++) {
                ...
                const wallBottomY = Math.floor((ratio) * (this.player.playerHeight - castedBlock.ceilHeight) + this.screenMiddleHeight);
                const wallTopY = wallBottomY + castedBlockHeight;
                ...
                const oppositeStartY = Math.floor((oppositeRatio) * (this.player.playerHeight - castedBlock.ceilHeight) + this.screenMiddleHeight);
                const oppositeCastedBlockHeight = oppositeRatio * cellHeight;
                const oppositeEndY = oppositeStartY + oppositeCastedBlockHeight;
                const oppositeSlicer = castedBlock.oppositeSide === 0 ? castedBlock.wallYOpposite : castedBlock.wallXOpposite;
                //this.#drawWall(bufferX, Math.floor(oppositeStartY), Math.floor(oppositeEndY), oppositeCastedBlockHeight, castedBlock.wall, oppositeSlicer, castedBlock.correctedRayDistanceOpposite, castedBlock.ceilHeight, castedBlock.floorHeight, oppositeStartY, oppositeCastedBlockHeight);
            ...
            this.#drawCeil(bufferX, rayAngle, ray.perpDistanceCorrector, { min: 0, max: startY });
            this.#drawFloor(bufferX, rayAngle, ray.perpDistanceCorrector, { min: Math.floor(startY + castedBlockHeight), max: this.SCENE_HEIGHT });
        }
    ...
    #castSingleRay(angle) {
        ...
        while (mapTraverse) {
            ...
            if (!mapTraverse) {
                ...
                */
                castedBlocks.push({ ...hitValue, mapX, mapY, side});
                ...
                if (castedTile.blocked) {
                    ...
                    */
                    ...
                    if (!previousTile?.blocked || (previousTile?.ceilHeight !== castedTile.ceilHeight || previousTile?.floorHeight !== castedTile.floorHeight)) {
                        if (castedTile.slices) {
                            for (const slice of castedTile.slices) {
                                castedBlocks.push({ ...castedTile, mapX, mapY, side, ceilHeight: slice.zMax, floorHeight: slice.zMin });
                            }
                        } else {
                            let oppositeSideX, oppositeSideY, oppositeSide;
                            if (sideDistX < sideDistY) {
                                oppositeSideX = mapX + stepX;
                                oppositeSide = 0;
                            } else {
                                oppositeSideY = mapY + stepY;
                                oppositeSide = 1;
                            }
                            castedBlocks.push({ ...castedTile, mapX, mapY, side, oppositeSideX, oppositeSideY, oppositeSide});
                        }
                    }
        ...
        let wallX, wallY, perpWallDist, perpWallDistOpposite, wallXOpposite, wallYOpposite;
        ...
        for (const castedBlock of castedBlocks) {   
            if (castedBlock.side === 0) {
                ...
                const intersectOppositeX = castedBlock.oppositeSideX + (stepX < 0 ? 1 : 0);
                perpWallDist = (intersectX - this.player.coordX) / rayDirX;
                perpWallDistOpposite = (intersectOppositeX - this.player.coordX) / rayDirX;
                wallX = intersectX;
                wallY = this.player.coordY + perpWallDist * rayDirY;
                wallXOpposite = intersectOppositeX;
                wallYOpposite = this.player.coordY + perpWallDistOpposite * rayDirY;
            } else {
                const intersectY = castedBlock.mapY + (stepY < 0 ? 1 : 0);
                const intersectOppositeY = castedBlock.oppositeSideY + (stepY < 0 ? 1 : 0);
                perpWallDistOpposite = (intersectOppositeY - this.player.coordY) / rayDirY;
                perpWallDist = (intersectY - this.player.coordY) / rayDirY;
                wallY = intersectY;
                wallX = this.player.coordX + perpWallDist * rayDirX;
                wallYOpposite = intersectOppositeY;
                wallXOpposite = this.player.coordX + perpWallDistOpposite * rayDirX;
            }

            castedBlock.correctedRayDistance = (perpWallDist * perpDistanceCorrector);
            castedBlock.wallX = wallX;
            castedBlock.wallY = wallY;
            castedBlock.wallXOpposite = wallXOpposite;
            castedBlock.wallYOpposite = wallYOpposite;
            castedBlock.correctedRayDistanceOpposite = (perpWallDistOpposite * perpDistanceCorrector);
        }
