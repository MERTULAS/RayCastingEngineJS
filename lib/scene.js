...
class Scene {
    ...
    #drawWall(x, y, color, height, hitValue, slicer, startY, correctedRayDistance) {
        ...
        const wallTexture = this.textureManager.textures.get(hitValue);

        for (let y = startY, texY = 0; y < endY; y++, texY++) {
            const textureY = Math.floor(((texY / height) * 32));
            const color = wallTexture.getPixelData(textureX, textureY, brightness);
            this.sceneBuffer[y * this.SCENE_WIDTH + x] = color;
        }
    }

    #drawFloor(bufferX, rayAngle, distanceCorrector, scanRange) {
        for (let y = scanRange.min; y < scanRange.max; y++) {
            ...
            const realDistance = rowDistance / distanceCorrector;
            ...
            const color = floorTexture.getPixelData(textureX, textureY, brightness);
            this.sceneBuffer[y * this.SCENE_WIDTH + bufferX] = color;
        }
    }

    #drawCeil(bufferX, rayAngle, distanceCorrector, scanRange) {
        for (let y = scanRange.min; y < scanRange.max; y++) {
            ...
            const brightness = this.#calculateBrightness(realDistance);
            const color = ceilTexture.getPixelData(textureX, textureY, brightness);
            this.sceneBuffer[y * this.SCENE_WIDTH + bufferX] = color;
        }
    }
    ...
    render() {
        ...
        for (let i = 0, bufferX = 0; i < this.player.fieldOfViewDeg; i += this.rayStep, bufferX++) {
            ...
            const correctedRayDistance = (ray.distance * ray.distanceCorrector);
            ...
            this.#drawCeil(bufferX, rayAngle, ray.distanceCorrector, { min: 0, max: startY });
            this.#drawWall(bufferX, startY, 0xFF000000, height, ray.hitValue, slicer, startY, correctedRayDistance);
            this.#drawFloor(bufferX, rayAngle, ray.distanceCorrector, { min: Math.floor(startY + height), max: this.SCENE_HEIGHT });
        }
    ...
    #castSingleRay(angle) {
        ...
        const distanceCorrector = Math.cos((angle - this.player.rotateX) * RADIUS);
        ...
        return {
            ...
            angle: angle,
            side: side,
            distanceCorrector: distanceCorrector
        };
    }
